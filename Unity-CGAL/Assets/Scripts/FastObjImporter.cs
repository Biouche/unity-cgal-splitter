...
public sealed class FastObjImporter
{
	...
	public Mesh ImportFile(string filePath)
	{
		...
		{
			if (!newVerts.Contains (vertices [faceData [i].x - 1])) {
				newVerts.Add (vertices [faceData [i].x - 1]);
			} else {
				Debug.Log ("Duplicate of " + vertices [faceData [i].x - 1]);
				continue;
			}
			if (faceData[i].y >= 1)
				newUVs.Add( uv[faceData[i].y - 1]);

			if (faceData[i].z >= 1)
				newNormals.Add(normals[faceData[i].z - 1]);
		}
